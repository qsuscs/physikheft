...
\makeatletter\def\itemb{\@ifnextchar[{\itembb}{\itembb[\@itemlabel ]}}
\def\itembb[#1]#2{\item[#1]\fbox{#2}}\makeatother
...
\item
	\( P = U \cdot I \)
\item
	\( \Delta \E = P \cdot \Delta t \quad \Rightarrow \quad P = \frac{\Delta
	\E}{\Delta t} \) bzw.\ \( P=\dot{\E} \)
\item \fbox{
	\( \E = qU \)
}
\item
...
\begin{itemize}
\itemb{
	\( C = \frac{Q}{U} \quad \Rightarrow \quad Q = CU \) } (merke: Die Kuh
	bleibt immer gleich)
\itemb{
...
\item
	...
	\fbox{\( B = \mu_0 \cdot \frac{I}{2 \pi r} \)}
\item
	\fbox{\( B = \mu_0 \mu_R \cdot I \cdot \frac{n}{l} \)} (lange Spule)
\item
...
\item
	\fbox{\( \dot{\Phi} = \dot{A} \cdot B + A \cdot \dot{B} \)}; \quad (\(
...
\item
	\fbox{\( \lambda_k = 2l/(k+1) \quad k \in \mathbb{N} \)}\\
	(Wellenlängenbedingung bei gleichen Enden)
\item
	\fbox{\( \lambda_k = 4l/(2k+1) \quad k \in \mathbb{N} \)}\\
	(ein freies und ein festes Ende)
\item
	\fbox{\( f_k = \frac{c}{\lambda_k} \quad k \in \mathbb{N} \)}
\end{itemize}

\subsection{Schwebung}
\begin{itemize}
\item
	\fbox{\( \Delta f_s = f_1 - f_2 \)} (Schwebungsfrequenzbereich)
\item
	\fbox{\( f_s = \frac{f_1 + f_2}{2} \)} (Frequenz der Schwebung)
\end{itemize}
...
\begin{itemize}
\item
	\fbox{\( a_k = \frac{\Delta s}{d} \cdot l \)};\quad\( \Delta s = k \cdot
