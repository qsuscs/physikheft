...
\paragraph{Beobachtung:} Auf dem Schirm erkennt man wieder eine Abfolge von
Maxima und Minima. Die Helligkeit der Maxima nimmt vom Hauptmaximum in der
Mitte nach außen hin schnell ab.

\paragraph{Idee:} Die Teilbündel 1 und 2 interferieren vollständig desktruktiv,
wenn zwischen den entsprechenden Randstrahlen der Bündel ein Gangunterschied
von \(\frac{\lambda}{2}\)

\paragraph{Ergebnis:}
\begin{itemize}
	\item Das Hauptmaximum liegt bei \(\Delta s = 0 \lambda\)
	\item \emph{Minima} sind bei \(\Delta s = \lambda;\, 2\lambda;\,
		3\lambda;\, ...\)
	\item Neben\emph{maxima} sind bei \(\Delta s = \frac{\lambda}{2};\,
		\frac{3\lambda}{2};\, \frac{5\lambda}{2};\, ...\)
\end{itemize}

\subsection{Das "`Gitter"'}
Ein \emph{Gitter} ist gewissermaßen ein Vielfachspalt mit sehr schmalen, sehr
eng beeinander liegenden Spalten. Hier wird nicht mehr die Spaltanzahl, sondern
die "`Anzahl der Striche"' pro mm als sogenannte \emph{Gitterkonstante}
angegeben.\\
Beispiel: "`140 Striche pro mm"' \(\rightarrow\) \(g = \frac{1
\mathrm{mm}}{140} = \frac{1}{140} \mathrm{mm}\)\\
Die Beobachtung der Interferenzbilder zeigt wenige scharfe Maxima mit großen
Abständen. Die Anzahl der sichtbaren Maxima ist in Abhängigkeit von \(g\)
beschränkt. Für Gitter gilt unsere Doppelspaltnäherung \(\sin(\alpha) =
\tan(\alpha)\) \emph{nicht} mehr, wir müssen über den Winkel rechnen:
\end{document}
